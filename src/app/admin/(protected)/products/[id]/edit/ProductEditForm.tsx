...
export function ProductEditForm({
  ...
  return (
    <>
      {state?.ok && <Toast key={toastKey.current} message="保存成功 ✓" type="success" />}
      {state && !state.ok && <Toast key={state.error} message={state.error} type="error" />}
        ...
        <form action={formAction} className="space-y-6">
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <CoverImagePicker media={media} defaultMediaId={product.coverMediaId ?? ""} />
